        ...
        {bookInfos.thumbnail ? (
          <>
            <div className='lg:row-span-1'>
              ...
              <div key={bookInfos.authors} className='pb-2 font-bold tracking-tight'>
                Un livre de {bookInfos.authors}
              </div>
              <p className='text-left pb-5 tracking-tight'>Année de publication :{bookInfos?.publishedDate ? ' ' + bookInfos?.publishedDate.substring(0, 4) : ' inconnue'}</p>
          ...
          <>
            <Link href={`/books/details/${bookInfos.id}`}>
              <p className='hover:scale-125'>
                <Image src={genBook} alt={bookInfos.title} height={200} width={150} />
              </p>
            </Link>
            <Link href={`/books/details/${bookInfos.id}`}>
