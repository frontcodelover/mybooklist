    ...
    <div className="relative px-4 pt-6 sm:px-6 lg:px-8">
      <nav className="relative flex items-center justify-between sm:h-10 lg:justify-start" aria-label="Global">
        ...
        <div className="hidden md:ml-10 md:flex md:space-x-8 md:pr-4">
          {navigation.map((item) => (
            <a key={item.name} href={item.href} className="font-semibold text-main-color hover:text-purple-700">
              {item.name}
            </a>
          ))}
          <Search />
          <a href="#" className="font-medium text-white hover:bg-purple-700 bg-purple-500 py-2 px-4 rounded-full">
            Se connecter
          </a>
        </div>
      </nav>
